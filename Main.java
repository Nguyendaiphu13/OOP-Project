...
public class Main extends Application {
    ...
    private void setupGameObjects() {
        ...
        gameManager.paddle = new Paddle(paddleX, paddleY, paddleWidth, paddleHeight,0,0,0);


        int ballRadius = 10;
        double ballX = SCREEN_WIDTH / 2.0 - ballRadius;
        double ballY = SCREEN_HEIGHT / 2.0 - ballRadius;
        gameManager.ball = new Ball(ballX, ballY, ballRadius * 2, ballRadius * 2, 4.0, 1, -1, ballRadius); // Tốc độ 4.0
    ...
    private void updateGame() {
        ...
        gameManager.ball.checkWallCollision(SCREEN_WIDTH, SCREEN_HEIGHT);
        gameManager.ball.checkCollision(gameManager.paddle);

        Iterator<Brick> brickIterator = gameManager.bricks.iterator();
        while (brickIterator.hasNext()) {
            Brick brick = brickIterator.next();
            if (gameManager.ball.checkCollision(brick)) {
                brick.takeHit();
        ...
        if (!gameManager.ball.alive) {
            ...
            if (gameManager.lives <= 0) {
                ...
                gameManager.ball.setX(SCREEN_WIDTH / 2.0 - gameManager.ball.getWidth() / 2.0);
                gameManager.ball.setY(SCREEN_HEIGHT / 2.0 - gameManager.ball.getHeight() / 2.0);
