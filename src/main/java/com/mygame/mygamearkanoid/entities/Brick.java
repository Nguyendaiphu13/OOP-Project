...
// SỬA Ở ĐÂY: Kế thừa từ MovableObject
public class Brick extends MovableObject {
    ...
    public Brick(double x, double y, int width, int height, String type) {
        // SỬA Ở ĐÂY: Gọi super() của MovableObject (loại không có dx, dy)
        // Điều này sẽ tự động đặt dx=0, dy=0
    ...
    private Color getColor() {
        return switch (this.type) {
            case "2normal" -> (this.hitPoints == 2) ? Color.FIREBRICK : Color.ORANGE;
            case "expand" -> Color.CYAN;
            // THÊM Ở ĐÂY: Màu dự phòng
            case "fast_ball" -> Color.YELLOW;
            default -> Color.GREEN;
        };
    }
